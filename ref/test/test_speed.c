...
int main(void)
{
  ...
  for(i=0;i<NTESTS;i++) {
    t[i] = cpucycles();
    gen_matrix_ring(matrix, seed);
  }
  print_results("gen_a: ", t, NTESTS);

  for(i=0;i<NTESTS;i++) {
    t[i] = cpucycles();
    poly_getnoise_eta1(&ep, seed, 0);
  }
  print_results("poly_getnoise_eta1: ", t, NTESTS);

  for(i=0;i<NTESTS;i++) {
    t[i] = cpucycles();
    poly_getnoise_eta2(&ep, seed, 0);
  }
  ...
  for(i=0;i<NTESTS;i++) {
    t[i] = cpucycles();
    poly_invntt_tomont(&ep);
  }
  print_results("INVNTT: ", t, NTESTS);

  for(i=0;i<NTESTS;i++) {
    t[i] = cpucycles();
    poly_basemul_montgomery(&ep, &matrix[0].vec[0], &matrix[1].vec[0]);
  }
  print_results("poly_basemul_montgomery: ", t, NTESTS);

  for(i=0;i<NTESTS;i++) {
    t[i] = cpucycles();
    polyvec_basemul_acc_montgomery(&ep, &matrix[0], &matrix[1]);
  }
